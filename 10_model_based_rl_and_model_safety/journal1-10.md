...
Modules 7–8 focused on architectural and algorithmic refinements to DQN with splitting the Q-network into separate value and advantage streams, improving efficiency since many states don't require knowing every action's value. Distributed RL focused on parallelizing actors and learners to scale experience collection. Then we went to policy gradient methods, where the policy itself is parametrized and optimized directly rather than derived greedily from Q-values (action-value based). REINFORCE introduced this approach taking an MC approach, but required baselines to reduce gradient variance, naturally leading to actor-critic methods that combine a learned policy with a value function baseline.

Modules 9–10 deepened actor-critic methods and introduced trust region approaches. TRPO and PPO both prevent policy updates from being too aggressive and destabilizing—TRPO through hard KL-divergence constraints, PPO through simpler clipped objectives that have become standard practice. The final module stepped back to model-based RL, exploring how agents can learn environment models for planning instead of relying purely on real interaction. The key takeaway was a reframing of safety: rather than optimizing reward, the focus shifted to identifying catastrophic states to avoid, especially relevant for domains like healthcare where exploration has real irreversible costs.
```
